...
async function run() {
  try {
    ...
    // Let's get logs for the last 24 hours to be safe
    const endTime = Date.now();
    const startTime = endTime - 24 * 3600000; // 24 hours ago

    for (const dev of devices) {
      console.log(`\n=================== Device logs: ${dev.name} (${dev.id}) ===================`);
      const path = `/v1.0/devices/${dev.id}/logs?start_time=${startTime}&end_time=${endTime}&size=50`;
      const logsResult = await queryEndpoint(token, path);
