...
async function sendTuyaCommand(accessToken, deviceId, code, value) {
  const t = Date.now().toString();
...
async function run() {
  try {
    ...
    const testCases = [
      { code: 'reset', value: 'force_reset' },
      { code: 'reset', value: 'reset' },
      { code: 'reset', value: true },
      { code: 'reset', value: 'force' },
      { code: 'RESET', value: 'force_reset' },
      { code: 'RESET', value: 'Force Reset' }
    ];

    for (const testCase of testCases) {
      console.log(`Sending command: code="${testCase.code}", value=${JSON.stringify(testCase.value)}...`);
      const result = await sendTuyaCommand(token, deviceId, testCase.code, testCase.value);
