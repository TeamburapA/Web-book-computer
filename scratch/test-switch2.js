...
async function sendTuyaCommand(accessToken, deviceId, code, value) {
  ...
  const contentHash = crypto.createHash('sha256').update(body).digest('hex');
  const stringToSign = [method = 'POST', contentHash, '', path].join('\n');
...
async function run() {
  try {
    ...
    console.log('Sending switch_2 command...');
    const result = await sendTuyaCommand(token, deviceId, 'switch_2', true);
    console.log('Result for switch_2:', JSON.stringify(result, null, 2));
  } catch (err) {
