...
async function run() {
  try {
    ...
    console.log('\n--- Functions ---');
    const functionsResult = await queryEndpoint(token, `/v1.0/devices/${deviceId}/functions`);
    console.log(JSON.stringify(functionsResult, null, 2));

    console.log('\n--- Specifications ---');
    const specsResult = await queryEndpoint(token, `/v1.0/devices/${deviceId}/specifications`);
